        ...
        <div class="bg-white border-b border-gray-200 py-6 sm:px-6 lg:px-8 px-5">
            <div class="md:flex md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                    <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Faults') }}</h1>
                </div>
                <div class="mt-4 flex md:mt-0 md:ml-4">
                    <router-link
                        class="btn btn-blue shadow-sm rounded-md mr-4"
                        to="/dashboard/tickets/new"
                        v-if="userHasPermission_FromMixin('CAN_CREATE_NEW_TICKET')"
                    >
                        {{ $t('New Fault Ticket') }}
                    </router-link>
                                                                    ...
                                                                    <template v-slot:selectOption="props">
                                                                        <div class="flex items-center space-x-3">
                                                                            <div class="flex-shrink-0 inline-block">
                                                                                ...
                                                                                v-if="props != null && props.option != null"
                                                                                >
                                                                            </div>
                                                            ...
                                                            <div class="col-span-3 mb-2">
                                                                ...
                                                                <div class="relative inline-flex w-full">
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <option value="department_id">{{ $t('Organization') }}</option>
                                ...
                                >
                                    <div class="flex items-center truncate space-x-3">
                                        <img
                                            ...
                                            v-if="ticket != null && ticket.user != null"
                                        >
                ...
                <div class="hidden sm:block">
                    <div class="align-middle inline-block min-w-full border-b border-gray-200">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead>
                            <tr>
                                <th class="px-3 pt-2 pb-3" v-if="false">
                                    <input
                                ...
                                <th class="px-3 py-2 text-left text-xs leading-4 font-medium text-gray-600 uppercase tracking-wider whitespace-no-wrap overflow-x-auto">Fault#</th>
                            ...
                            <template v-for="ticket in ticketList">
                                <router-link
                                    ...
                                    tag="tr"
                                >
                                    <td class="px-3 py-4 whitespace-no-wrap text-center text-sm leading-5 font-medium" v-if="false">
                                        <input
                                            :id="'ticket-' + ticket.id"
                                        ...
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                ...
                                                <div class="text-sm leading-5 text-gray-500">
                                                    ...
                                                    <table class="sdl_admin_table">
                                                        <tr>
                                                            <td>Assigned to:</td>
                                                            <td>{{ (ticket.user != null) ? ticket.user.name : '' }}</td>
                                                        </tr>
                                                        <!-- <tr>
                                                            <td>Email</td>
                                                            <td>{{ (ticket.user != null) ? ticket.user.email : '' }}</td>
                                                        </tr> -->
                                                        <tr v-if="ticket.department && ticket.department.name">
                                                            <td>Organization</td>
                                                            <td>{{ ticket.department.name }}</td>
                                                        </tr>
                                                        <tr v-if="ticket.sub_department && ticket.sub_department.name">
                                                            <td>Project</td>
                                                            <td>{{ ticket.sub_department.name }}</td>
                                                        </tr>
                                                        <tr v-if="ticket.department_location && ticket.department_location.location_name">
                                                            <td>Location</td>
                                                            <td>{{ ticket.department_location.location_name }}</td>
                                                        </tr>
                                                        <tr v-if="ticket.department_sub_location && ticket.department_sub_location.location_name">
                                                            <td>Sub Location</td>
                                                            <td>{{ ticket.department_sub_location.location_name }}</td>
                                                        </tr>
                                                        <tr v-if="ticket.type && ticket.type.id">
                                                            <td>Fault type</td>
...
export default {
    ...
    mounted() {
        // filters.statuses
        if( this.$route.query.statuses != undefined ){
            this.filters.statuses = [];
            // console.log('this.$route.query.statuses--', this.$route.query) // outputs 'yay'
            this.$route.query.statuses = this.$route.query.statuses.split(',');
            for(let x in this.$route.query.statuses){
                console.log( this.$route.query.statuses[x] );
                this.filters.statuses.push( this.$route.query.statuses[x] );
            }
        }
        let self = this;
        setTimeout(function(){
    ...
    watch: {
        '$route'(to,from) {
            // console.log("in @/views/pages/dashboard/tickets/list, route change", to, from);
            //Since same component is loading for both path, we need to reload the page for it
    ...
    methods: {
        ...
        getTickets() {
            ...
            //only my tickets
            let only_my_tickets = 0;
            if(this.$route.path == "/dashboard/tickets-my"){
                only_my_tickets = 1;
            }
            ...
            axios.get('api/dashboard/tickets', {
                params: {
                    ...
                    only_my_tickets: only_my_tickets
                }
            }).then(function (response) {
                // console.log('dashboard ticket response', response);
                // return 1;
        ...
        setDepartmentListForOptionAndOptionGroup(departmentList){
            let departmentListNew = departmentList;
            for(let x in departmentList){
                let dRow = departmentList[x];
                if(dRow.subDepartments != undefined && dRow.subDepartments.length > 0){
                    dRow.options = [];
                    for(let y in dRow.subDepartments){
                        let sdRow = dRow.subDepartments[y];
                        let tempOption = {
                                'id': sdRow.id,   
                                'name': sdRow.name,
                            };
                        dRow.options.push(tempOption);
                        // console.log('tempOption', tempOption);

                        departmentListNew.push(tempOption);
                    }
                }

                // departmentList[x] = dRow;
        ...
        getFilters() {
            const self = this;
            axios.get('api/dashboard/tickets/filters?called_from=dashboard_tic').then(function (response) {
                self.agentList = response.data.agents;
                self.departmentList = response.data.departments;
                self.setDepartmentListForOptionAndOptionGroup(self.departmentList);
                self.labelList = response.data.labels;
                self.statusList = response.data.statuses;
                self.allowedStatusList = self.fetchAllowedStatusList_FromMixin( response.data.statuses );
...
.sdl_admin_table tr td:first-child{
    font-weight: bold;
    width: 100px;
}
</style>
