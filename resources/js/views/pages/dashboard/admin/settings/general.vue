                    ...
                    <div class="shadow sm:rounded-md sm:overflow-hidden">
                        <div class="px-4 py-5 bg-white sm:p-6">
                            <div class="grid grid-cols-3 gap-6">
                                ...
                                <div class="col-span-3">
                                    <label class="block text-sm font-medium leading-5 text-gray-700" for="kpi_value">{{ $t('KPI value') }}</label>
                                    <div class="mt-1 relative rounded-md shadow-sm">
                                        <input
                                            id="kpi_value"
                                            v-model="kpi_value"
                                            :placeholder="$t('kpi value(task per day)')"
                                            class="form-input block w-full transition duration-150 ease-in-out sm:text-sm sm:leading-5"
                                            required
                                        >
                                    </div>
    ...
    methods: {
        get() {
            ...
            axios.get('api/dashboard/admin/settings/general').then(function (response) {
                ...
                self.kpi_value = response.data.kpi_value;                
                self.loading = false;
            }).catch(function () {
