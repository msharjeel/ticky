    ...
    <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
            ...
            <div class="md:flex md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                    <h1 class="py-0.5 text-2xl font-semibold text-gray-900">
                        {{ parent_id > 0 ? $t('Projects') : $t('Organizations') }}
                    </h1>
                </div>
                <div class="mt-4 flex md:mt-0 md:ml-4">
                    ...
                    >
                        {{ parent_id > 0 ? $t('Create project') : $t('Create organization') }}
                    </router-link>
                </div>
            </div>
        </div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="my-6 bg-white shadow overflow-hidden sm:rounded-md">
                ...
                <template v-if="departmentList != undefined && departmentList.length > 0">
                    <ul>
                        <template v-for="(department, index) in departmentList">
                            <li :class="{'border-t border-gray-200': index !== 0}">
                                <div class="px-4 py-4 flex items-center sm:px-6">
                                    <div class="min-w-0 flex-1 sm:flex sm:items-center sm:justify-between">
                                        <div>
                                            ...
                                            >
                                                ...
                                                {{ $t('The organization is not public') }}
                                            </div>
                                        </div>
                                        <div class="mt-4 flex-shrink-0 sm:mt-0">
                                            <div class="flex overflow-hidden">
                                                ...
                                                <template v-else-if="department.all_agents && department.agents != null && department.agents.length === 0">
                                                    <div class="text-sm text-gray-700">
                                    ...
                                    <router-link
                                        :to="'/dashboard/admin/organizations/' + department.id + '/edit'"
                                        class="block hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
                                    >Edit</router-link>
                                    
                                    
                                    <template v-if="department.parent_id < 1">

                                        &nbsp | &nbsp 

                                        <router-link
                                            :to="'/dashboard/admin/locations-of-organization/' + department.id"
                                            class="block hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
                                        >Main Locations ( {{ (department.departmentLocation != undefined) ? department.departmentLocation.length : '0' }} )</router-link>

                                        &nbsp | &nbsp 

                                        <router-link
                                            :to="'/dashboard/admin/sub-organizations-of/' + department.id"
                                            class="block hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"                                        
                                        >Projects ( {{ (department.subDepartments != undefined) ? department.subDepartments.length : '0' }} )</router-link>
                                    </template>
...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Organization')+'s'
        }
    },
    data() {
        return {
    ...
    mounted() {
        ...
        // console.log('mounted-',this.$route.query, this.$router.params, this.$router, this.$route.params.id);
    },
    // created() {
    ...
    methods: {
        ...
        getDepartments() {
            // console.log('mounted-',this.$route.query, this.$router.params, this.$router, this.$route.params.parent_id);

            let get_departments_url = 'api/dashboard/admin/departments';
            
            if(this.parent_id > 0){
                get_departments_url = 'api/dashboard/admin/sub-departments-of/'+this.parent_id;
            }

            const self = this;
            self.loading = true;
            axios.get(get_departments_url).then(function (response) {
