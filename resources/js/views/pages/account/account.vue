                ...
                <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    ...
                    <div class="mt-10 shadow sm:rounded-lg">
                        <form @submit.prevent="changePassword">
                            ...
                            <div class="bg-white md:grid md:grid-cols-3 md:gap-6 px-4 py-5 sm:p-6">
                                <div class="md:col-span-1">
                                    <h3 id="change_password" class="text-lg font-medium leading-6 text-gray-900">{{ $t('Change password') }}</h3>
                                    <p class="mt-1 text-sm leading-5 text-gray-500">
...
export default {
    ...
    mounted() {
        ...
        if(this.$route.query.msg_code == 'utp'){
            let $cp_div = $("#change_password");
            $('html, body').animate({
                scrollTop: $cp_div.offset().top
            }, 2000);

            $cp_div.css('background-color','lightyellow');
            setTimeout(function(){
                $cp_div.css('background-color','unset');
            },3000);

            this.updateFlagForPasswordChange();
        }//end work of: update temporary password
    },
    data() {
        return {
            loading: {
    ...
    methods: {
        updateFlagForPasswordChange(){
            const self = this; 
            axios.post('api/account/updateFlagForPasswordChange',{}).then(function (response) {
                self.$store.state.user.notified_for_password_change = 1;
            }).catch(function () {                
            });
        },
        getUser() {
