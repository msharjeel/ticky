                        ...
                        <div class="bg-white md:grid md:grid-cols-3 px-4 py-5">
                            <div class="md:col-span-2">
                                <div class="grid grid-cols-3 gap-6">
                                    <template v-if="allDepartment.length > 0">
                                        <div v-if="$store.state.user.department_id > 0" class="col-span-3">
                                            ...
                                            <label class="mt-5 block text-sm font-medium leading-5 text-gray-700" for="sub_department">{{ $t('Project') }}</label>
                                    ...
                                    <div class="col-span-3">
                                        <label class="block text-sm font-medium leading-5 text-gray-700" for="priority_id">{{ $t('Priority') }}</label>
                                        <div class="mt-1 relative rounded-md shadow-sm">
                                            <input-select
                                                id="priority_id"
...
export default {
    ...
    mounted() {
        // this.getDepartments();
        this.getFilters();
        this.ask_question_on_page_leave = true;
        
        if(this.$store.state.user.department_id > 0){
    ...
    beforeRouteLeave (to, from , next) {
        if(this.ask_question_on_page_leave){
              const answer = window.confirm('Are you sure to discart ticket ? ')
              if (answer) {
                next()
              } else {
                next(false)
    ...
    methods: {
        ...
        getFilters() {
            ...
            axios.get('api/tickets/filters?call_from=ticket_new').then(function (response) {
        ...
        /*
        getDepartments() {
            return true; //now doing this work in filters request

            const self = this;
            self.loading.form = true;
            axios.get('api/tickets/departments?call_from=ticket_new').then(function (response) {
                self.allDepartment = response.data;                
                self.departmentList = self.onlyParentDepartments();
                self.subDepartmentsList = self.allDepartment;
                self.onDepartmentDropDownChange(null);
        ...
        onDepartmentLocationChange($event){
            console.log('onDepartmentLocationChange', this.ticket.department_location_id, this.departmentLocationList);
        ...
        saveTicket() {
            ...
            axios.post('api/tickets', self.ticket).then(function (response) {
                ...
                self.ask_question_on_page_leave = false;
