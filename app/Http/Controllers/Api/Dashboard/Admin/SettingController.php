...
class SettingController extends Controller
{
    ...
    public function getGeneral(): JsonResponse
    {
        return response()->json([
            ...
            'kpi_value' => Helper::getValueFromSettings('kpi_value'),
            // 'kpi_value' => Setting::getDecoded('kpi_value'),
        ]);
    }
    ...
    public function setGeneral(GeneralUpdateRequest $request): JsonResponse
    {
        ...
        $settings = Setting::find('kpi_value');
        $settings->value = $request->get('kpi_value');
        if (!$settings->save()) {
            return response()->json(['message' => __('An error occurred while saving kpi value')], 500);
        }
