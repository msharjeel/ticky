...
class Ticket extends Model
{
    ...
    public function departmentLocation(): BelongsTo
    {
        return $this->belongsTo(DepartmentLocation::class, 'department_location_id', 'id');
    }

    public function departmentSubLocation(): BelongsTo
    {
    ...
    public function ticketBody(): HasMany
    {
        return $this->hasMany(TicketReply::class)->orderBy('id', 'asc')->limit(1);
    }
    ...
    public function verifyUser(User $user): bool
    {
        if ($user->role_id !== 1) {
            ...
            return 
             ...
             //($this->department->all_agents || $this->department->agent()->pluck('id')->contains($userId)) ||
             // ( in_array($userId, [$this->user_id, $this->agent_id, $this->created_by, $this->closed_by]) )
            ( in_array($userId,Helper::getTicketStakeHolderIds($this)) )
            ;
        }
        return true;
    }
